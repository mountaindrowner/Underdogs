...
- **Effect verbs (added for Set 1 content):** `auraBuff` (continuous in `aura`; with `grantToTarget` grants an aura), `returnToHand`, `shuffleIntoDeck`, `delayedTransform` (Joseph), `conditionalDeal`, `gainForEachSheep`, `discoverFromDeck`, `onHealBonus` (passive), `discountHand` (Israel/Terah cost auras), `gainProvision` (the Loaf of Bread token: +N Provision to spend this turn). Triggers beyond the core five: `startOfTurn`, `endOfTurn`, `onDeath` (+`replaceDeath`), `raise` (ally-death listener), `trigger` (generic listener via `on`), `passive`.
- **Effect verbs / fields (added for the Neutral Expansion):** new op `condition`s `empty_hand` (Gleaner, Jar of Flour & Oil) and `singleton_deck` (highlander — Lone Pilgrim); new passive verbs `requiresOtherPlay` (can't attack unless you played another card this turn — Eleventh-Hour Laborer; gated in the engine *and* the AI move-gen so it never stalls) and `breakAfterTurns` (a standing relic expires after N of your turns — the Jar of Oil, emits `relicBroken`); `buff` gains `perOtherAlly` (scale by your OTHER unit count — the Great Cloud of Witnesses); `returnFromDiscard` gains `toHand` + `diedThisTurn` (return a unit that died *this turn* to hand — Kinsman-Redeemer). New `PlayerState.cardsPlayedThisTurn` counter (reset each Dawn).
- **Effect verbs / fields (added for Set 2 legendaries):** `cannotAttackAlone` (passive: a unit can't attack while it's your only unit — Barak); `costReduce` graduated from placeholder to a real **cost-aura** (in `aura`, applied in `effCost`) that accepts a `filter:{cardType|class|tribe}` (Bezalel's Relics cost 1 less). New target selector `damagedEnemy` (mirror of `damagedAlly`, Jael). `buff`/effects now honor an op `filter:{tribe}` and `scope:'other'` on played effects (Mary buffs another Disciple). `returnFromDiscard` takes `count` + `withKeyword` (Nehemiah raises two, gives Guard). New aura condition `outnumbered`; new **Fulfill** conditions `hero_damaged` (Hezekiah/Caleb/Esther/Mordecai), `cast_spells` (Miriam/Habakkuk, via `PlayerState.spellsCast` − the unit's `spellsAtEntry`), and `outnumbered_win` (Gideon, via a `wonOutnumbered` flag set when a unit survives a fight while your board is smaller). **`node tools/audit-cards.ts` verifies every card's data against what the engine implements — run it whenever cards or the engine change; `node tools/soak.ts` plays AI-vs-AI across every class matchup as a crash test.**
...
Free. Currency = **Talents** (from wins, quests, campaign, achievements, and the Scroll Study loop). Packs = 5 cards, guaranteed ≥1 Rare+, Legendary within the first **10** packs and within every **20** after (D-45 tightened the earlier "~40-pack pity"); duplicate protection; surplus melts to **Fragments** for targeted crafting. **Scroll Study:** read a card's real passage in-app + 3 comprehension questions → capped daily Talent bonus; framed as "read the lore," never gates content. No IAP, no ads, no loot-box psychology.

**Built (v0.4.0, D-45 — spec in `docs/21-economy.md`):** ownership + starter Calling (+ a second-deck gift after chapter 1), the Storehouse with the pack-opening ritual, first-fruits packs, crafting/disenchanting, Daily Bread quests (3/day, one reroll), First Win of the Day, and a **soft daily taper** (~10 full-rate wins/day, then a 2-Talent trickle; losses always pay). Core is `ui/src/economy.ts` — deterministic persisted-LCG RNG, node-tested; keep it free of Vite-only imports. Scroll Study is still unbuilt.
...
- **Built & verified:** the deterministic seeded **rules engine** (`/engine`) + effect-verb interpreter driving all content from data; **182 collectible cards** (126 base + 30 Set 2 legendaries + 26 Neutral Expansion) **+ 15 Fulfilled forms + 12 leaders + 22 adversaries + 8 tokens**, every definition audited to actually execute its text; a full **front-end game loop** (Title → Menu → Story / Free Play / Decks → mulligan → match → result) on desktop and mobile-landscape; **The Armory** (browse/collection/forge) and **The Sparring Pit** (war-band + AI difficulty); interactive **Foresee/Discover** choices; music + SFX + haptics; the game-feel layer (directional lunge, damage-scaled shake, floats, summon bloom, dissolve-to-light deaths, Endure shatter, Fulfill burst, aura glow, board parallax — all `prefers-reduced-motion`-gated); AI with lethal awareness at every difficulty. Gatekeepers: `node --test engine/test/*.test.ts` (48 pass), `node tools/audit-cards.ts` (clean, 239 defs), `node tools/soak.ts` (AI-vs-AI, no crashes). *(Set 2's 45 cards + the 26 Neutral Expansion cards all have generated WebP art.)*
- **Built since (D-45):** the **economy** — Talents/Fragments, the Storehouse + pack ritual, pity + duplicate protection, crafting, Daily Bread, real collection ownership in the Armory/forge (spec: `docs/21-economy.md`; tests in `engine/test/economy.test.ts` — 57 total now pass).
- **Not built yet (the real gaps):** **Scroll Study**; the **sacred Cross/Resurrection interlude** (Law 5 — campaign is 4 combat chapters only); **iOS/Capacitor** wrapper; the economy's long tail (foils, Set/Blessed pack types, wishlist, weekly trials). See `STATUS.md` and `docs/06-decisions-log.md`.
