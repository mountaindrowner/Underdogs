...
### Content
- **156 collectible cards** (126 base + 30 Set 2 legendaries; schema-valid) plus **15 Fulfilled forms**, **12 leaders** w/ hero powers, **8 tokens** (incl. the Loaf of Bread), **22 adversaries** (all with authored effects now). All art present as WebP — the 45 Set 2 illustrations were generated by the `tools/cardart` pipeline (Fulfilled forms reference their early form for face-consistency).
...
0c. **Neutral expansion — BRIEF WRITTEN, not designed (`docs/19-neutral-set-prompt.md`).** A competitive-diversity pass: the game has only **21 neutrals** (10C/4R/1E/6L) with no top-end (nothing at cost 6–7), a thin 4–5 slot, almost no epics, and no neutral removal/AoE — so decks lack flexible options and answers. The doc is a self-contained handoff prompt for a design session (Laws, locked rules, the closed engine vocabulary, current pool, gaps, target ~24–28 cards at ~10C/8R/5E/4L incl. ~4 niche build-around legendaries, and the **"neutral paradox" guardrail**: neutrals go in every deck, so keep them conditional/build-around — not homogenizing stat-sticks — and tilt flexible power toward control). **Next:** hand the prompt to a design session; when cards return, audit → load into a new data file → wire five loaders → art → gatekeepers + `balance.ts`.
