...
0. **Balance — measured with a strong pilot (`tools/ai-eval.ts`, D-38).** A 2-ply rollout AI (`pickActionStrong`, in-game as the "Sage" difficulty) beats the greedy AI 88.6%, and re-measuring class win-rates with it **overturned the old "Warrior is just an AI artifact" read**: Warrior holds **83% under both greedy and strong play** — real card power, not myopia, so it warrants a trim. The strong AI *did* re-rate control (**Priest 46%→58%**, games longer — it was under-piloted before); **Prophet (25%) and Disciple (29%) are genuinely weak** under good play. **Next:** a larger strong-both run to firm the mid-pack, then a real balance pass (Warrior trim + Prophet/Disciple help) — not an AI fix. Prophet's face-reach (enemy-hero spell targeting) is now unblocked via Set 2's `deal→enemyHero`. Earlier applied pass: the **Loaf of Bread** seat fix (61.5%→48.3%), Warrior/Shepherd nerfs, Priest/Patriarch buffs (`docs/08-balance.md` Part C).
